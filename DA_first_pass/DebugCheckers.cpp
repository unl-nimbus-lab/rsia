...
//===----------------------------------------------------------------------===//
// ReachableConditionsDumper
//===----------------------------------------------------------------------===//

namespace {
class ReachableConditionsDumper : public Checker<check::ASTCodeBody> {
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager& mgr,
                        BugReporter &BR) const {
      ...
      if (ReachableConditions* RD = mgr.getAnalysis<ReachableConditions>(D)) {
          RD->dumpBlockConditions(mgr.getSourceManager());
		  //delete RD;
    }
  }
};
...
void ento::registerReachableConditionsDumper(CheckerManager &mgr) {
  mgr.registerChecker<ReachableConditionsDumper>();
}
...
namespace {
class RosPatternsDumper : public Checker<check::ASTCodeBody> {
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager& mgr,
                        BugReporter &BR) const {
      if (RosPatterns* RD = mgr.getAnalysis<RosPatterns>(D)) {
          RD->dump(mgr.getSourceManager());
		  //delete RD;
    }
  }
};
...
void ento::registerRosPatternsDumper(CheckerManager &mgr) {
  mgr.registerChecker<RosPatternsDumper>();
}

//===----------------------------------------------------------------------===//
// ExprScopeAnalysisDumper
//===----------------------------------------------------------------------===//

namespace {
class ExprScopeAnalysisDumper : public Checker<check::ASTCodeBody> {
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager& mgr,
                        BugReporter &BR) const {
      PrintingPolicy Policy(mgr.getLangOpts());
      ...
      D->print(llvm::errs(), Policy);
      llvm::errs() << "\033[0m\n";
      if (ExprScopeAnalysis* ECD = mgr.getAnalysis<ExprScopeAnalysis>(D)) {
          ECD->dumpBlockLocalness(mgr.getSourceManager());
		  //delete RD;
    }
  }
};
...
void ento::registerExprScopeAnalysisDumper(CheckerManager &mgr) {
  mgr.registerChecker<ExprScopeAnalysisDumper>();
}
